...
- Fixed problem accross multiple later chapters where tests passed but pytest reported `Can't add file tracer data for unmeasured file`
- Use `Git Bash` to test git installation on Windows on chapter 1
- Fixed the start of chapter 6 so it doesn't overlap with chapter 5
- Fix rendering problem in chapter 9
- Update the chapter 25 coverage report to match with the project generated by django-crash-starter
- Explained that TimeStampedModel is from django-model-utils, which is a third-party package in chapter 27
- Use `move` command to move the `cheeses` directory on Windows on chapter 27
- Use double quotes on some code samples of chapter 27
- Explained that shell_plus is from django-extensions which is a third-party package in chapter 28
- Fixed indentation on some code samples of chapter 28, 29 and 37
- Moved link to changelog from its own chapter to chapter 59
